...
const PricingCard = ({plan, billing}) => {
    ...
    const handleSelectPlan = () => {
        const  checkoutPath = `/checkout?pid=${plan.id}`
        ...
        return (
            ...
            <div className='group relative rounded-2xl overflow-hidden'>
                ...
                <Button text="Select Plan" variant={plan.featured ? "secondary" : "primary"} className="w-full" onClick={handleSelectPlan}/>
            ...
            {showLogin && (
                <Login onClose={() =>setShowLogin(false)} redirectTo={`/checkout?pid=${plan.id}`}/>
            )}
        </div>
